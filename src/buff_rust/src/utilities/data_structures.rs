...
impl RobotPose {
    ...
    pub fn new(xyz: Vec<f64>, rpy: Vec<f64>) -> RobotPose {
        RobotPose {
            position: xyz,
            rotation: rpy,
            velocity: vec![0.0; 3],
    ...
    pub fn update_from_speed(&mut self, dxyz: Vec<f64>, drpy: Vec<f64>, dt: f64) {
        ...
        self.position = dxyz
            ...
            .map(|(new_pos, pos)| (new_pos + pos) * dt / 2.0)
            .collect();
        self.rotation = drpy
            .iter()
            .zip(self.rotational_velocity.iter())
            .map(|(new_pos, pos)| (new_pos + pos) * dt / 2.0)
            .collect();
        self.velocity = dxyz;
        self.rotational_velocity = drpy;
    }
}
