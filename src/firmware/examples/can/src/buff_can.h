...
class BuffCan {
    ...
    public:
        ...
        // initializer
        void set_index(byte[MAX_NUM_RM_MOTORS][3]);

        // LUFs (look up functions)
        int8_t esc_id_from_motor_idx(int);
        int8_t can_bus_from_motor_idx(int);
        int8_t message_type_from_motor_idx(int);
        int8_t message_offset_from_motor_idx(int);
